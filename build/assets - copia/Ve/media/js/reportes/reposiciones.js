...
$(document).ready(function () {
	...
	function buscarReposiciones(paginaActual) {
		...
		if (validar_filtro_busqueda("lotes-2")) {
			...
			$consulta = $.post(baseURL + api + isoPais + "/reportes/reposiciones", {
				request: dataRequest,
				ceo_name: ceo_cook,
				plot: btoa(ceo_cook)
			});
			...
			$consulta.done(function (response) {
				...
				if (data.rc == "0") {
					...
					$(".Ve").addClass("td-medio");
					$(".tbody-SC").removeClass("tbody-SC");
					$("th").removeClass("elem-hidden");
					$.each(data.listadoReposiciones, function (posLista, itemLista) {
						...
						td.attr("style", "text-align: center");
						td = $(document.createElement("td")).appendTo(tr);
						td.html(itemLista.idLote);
						td.addClass("td-medio");
						td.attr("style", "text-align: center");
						td = $(document.createElement("td")).appendTo(tr);
						td.html(itemLista.idOrden);
						td.attr("style", "text-align: center");
						td = $(document.createElement("td")).appendTo(tr);
						td.html(itemLista.numFactura);
						td.attr("style", "text-align: center");
					});
					/*
			 			if (evBuscar) {
	...
	function paginar(totalPaginas, paginaActual) {
		$("#paginacion").paginate({
			...
			onChange     			: function(page){
										if( !$('.tabla-reportes tbody tr').hasClass('pg'+page) ){
													buscarReposiciones(page);
												}
												$('.tabla-reportes tbody tr').hide();
												$('.tabla-reportes .pg'+page).show();
