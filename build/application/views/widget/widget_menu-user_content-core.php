<?php defined('BASEPATH') OR exit('No direct script access allowed'); ?>
<nav class="main-nav main-nav-dropdown">
	<ul class="flex my-0 items-center list-style-none list-reset ">
		<li>
			<a class="mt-1 pl-2 pr-1 regular text-decoration-none white" href="#">Pedro Perez
				<i class="right pl-5 icon icon-chevron-down" aria-hidden="true"></i>
			</a>
			<ul class="dropdown regular tertiary bg-secondary">
				<li><a class="pl-2 pr-1 h6" href="#">Configuración</a></li>
				<li><a class="pl-2 pr-1 h6" href="<?= base_url('cerrar-sesion') ?>">Cerrar Sesión</a></li>
			</ul>
			<span></span>
		</li>
	</ul>
</nav>
