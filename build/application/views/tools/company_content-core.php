              ...
              <select id="idEnterpriseList" name="idEnterpriseList" class="select-box custom-select mt-3 mb-4 h6 w-100" countEnterpriseList=<?= $countEnterpriseList ?>>
                ...
                <?php else : ?>
                  ...
                  <?php foreach ($enterpriseSettList as $enterprise) : ?>
                    <option value="<?= $enterprise->acrif; ?>" idFiscal="<?= $enterprise->acrif; ?>" name="<?= $enterprise->acnomcia; ?>" businessName="<?= $enterprise->acrazonsocial; ?>" contact="<?= $enterprise->acpercontac; ?>" address="<?= $enterprise->acdirubica; ?>" billingAddress="<?= $enterprise->acdirenvio; ?>" phone1="<?= $enterprise->actel; ?>" phone2="<?= $enterprise->actel2; ?>" phone3="<?= $enterprise->actel3; ?>" countEnterpriseList="<?= $countEnterpriseList ?>">
            ...
            <?php if (lang('SETT_SETTINGS_TELEPHONES') === 'ON') : ?>
              <div class="row">
                <div id='divPhone1' class="form-group mb-3 col-6 col-lg-4 col-xl-4">
                  ...
                  <input type="text" id="phone1" name="phone1" class="form-control" maxlength="15" value="<?= $phone1; ?>" <?= $phoneUpdate; ?>>
                  <div class="help-block"></div>
                </div>

                <div id='divPhone2' class="form-group mb-3 col-6 col-lg-4 col-xl-4">
                  <label for="phone2"><?= novoLang(lang('GEN_TELEPHONE'), "2") ?></label>
                  <input id="phone2" name="phone2" class="form-control" maxlength="15" value="<?= $phone2; ?>" <?= $phoneUpdate; ?>>
                  <div class="help-block"></div>
                </div>

                <div id='divPhone3' class="form-group mb-3 col-6 col-lg-4 col-xl-4">
                  <label for="phone3"><?= novoLang(lang('GEN_TELEPHONE'), "3") ?></label>
                  <input id="phone3" name="phone3" class="form-control" maxlength="15" value="<?= $phone3; ?>" <?= $phoneUpdate; ?>>
                  <div class="help-block"></div>
                </div>
              </div>
              <?php
              if (lang('SETT_SETTINGS_ADDRESS_ENTERPRICE_UPDATE') === 'ON' || lang('SETT_SETTINGS_PHONES_UPDATE') === 'ON') :
              ?>
                <div class="row">
