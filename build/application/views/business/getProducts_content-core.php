  ...
  <div class="flex tertiary">
    <nav class="main-nav nav-inferior">
      <ul class="mb-0 h6 light tertiary list-style-none list-inline">
        <li class="inline"><a class="tertiary big-modal" href="<?= base_url(lang('SETT_LINK_ENTERPRISES')) ?>"><?= lang('GEN_MENU_ENTERPRISE') ?>
            /</a></li>
...
<div class="flex mt-3 items-center">
  <div class="flex h6 flex-auto justify-end">
    <div id="resetBtn" class="flex h6 flex-auto justify-end">
      <button class="btn btn-outline btn-small btn-rounded-left bg-white" data-jplist-control="reset" data-group="group-filter-pagination"
        data-name="reset"><?= lang('BUSINESS_PRODUCTS_ALL') ?></button>
    </div>
    <?php if(count($categories) > 1): ?>
    <select class="select-box custom-select mr-0 h6" data-jplist-control="select-filter" data-group="group-filter-pagination" data-name="category">
    ...
    <div id="sb-search" class="sb-search">
      <input data-jplist-control="textbox-filter" data-group="group-filter-pagination" data-name="description" data-path=".product-description"
        id="search" class="sb-search-input" type="search" name="search" value="" placeholder="Buscar...">
    ...
    <div data-jplist-group="group-filter-pagination" id="product-list" class="flex-auto">
      ...
      <div data-jplist-item class="select-product flex mb-1 pl-3 pr-4 py-1 bg-white justify-between items-center">
        <div class="flex mr-3 mx-1 items-center flex-auto filter-<?= $products->marca; ?> filter-<?= $products->idCategoria ?? ''; ?> ">
          <img class="img-product-list" src="<?= $this->asset->insertFile($products->productImg, 'images', $customerFiles, 'programs'); ?>"
            alt="<?= $products->productImg; ?>">
          <img class="mx-2 img-brand-list" src="<?= $this->asset->insertFile($products->imgBrand, 'images', $customerFiles, 'brands'); ?>"
            alt="<?= $products->imgBrand; ?>">
      ...
      <?php endforeach; ?>
      <div class="flex-auto my-5 py-4 center" style="display: none" data-jplist-control="no-results" data-group="group-filter-pagination"
        data-name="no-results">
        <span class="h4"><?= lang('GEN_TABLE_SZERORECORDS') ?></span>
      </div>
    </div>

    <div id="pagination-control" class="pagination page-number mb-5 py-5 flex-auto justify-center hide" data-jplist-control="pagination"
      data-group="group-filter-pagination" data-items-per-page="5" data-current-page="0" data-disabled-class="disabled"
      data-selected-class="page-current" data-name="pagination">
      <nav class="h4">
