...
class Servicios extends CI_Controller
{
  ...
  public function transferenciaMaestra($urlCountry)
  {
    ...
    if ($paisS == $urlCountry && $logged_in && $moduloAct !== false) {
      ...
      $FooterCustomInsertJS = [
        "jquery-3.6.0.min.js", "jquery-ui-1.13.1.min.js", "jquery.balloon.min.js",
      ...
      $header = $this->parser->parse('layouts/layout-header', array(
        'bodyclass' => '', 'menuHeaderActive' => TRUE,
        'menuHeaderMainActive' => TRUE, 'menuHeader' => $menuHeader, 'titlePage' => $titlePage
      ), TRUE);
      $footer = $this->parser->parse('layouts/layout-footer', array(
        ...
        'FooterCustomJSActive' => TRUE, 'FooterCustomJS' => $FooterCustomJS
      ), TRUE);
    ...
    if ($response) {
      if ($response->rc == 0) {
        unset(
          ...
          $response->className,
          $response->token,
          $response->idOperation,
          $response->logAccesoObject,
          $response->usuario
        );
  ...
  public function actualizarDatos($urlCountry)
  {
    ...
    if ($paisS == $urlCountry && $logged_in && $moduloAct !== false) {
      ...
      $footer = $this->parser->parse('layouts/layout-footer', array(
        'menuFooterActive' => TRUE, 'menuFooter' =>
  ...
  public function cargarArchivo($urlCountry)
  {
    ...
    if ($paisS == $urlCountry && $logged_in && $moduloAct !== false) {
      ...
      if (!$this->upload->do_upload()) {
        ...
        log_message('DEBUG', 'UPLOAD FILE TO: ' . BULK_FTP_HOST  . $urlBulkService . $nombreArchivo);
        ...
        if (ssh2_auth_pubkey_file($conn, BULK_FTP_USERNAME, BULK_FTP_KEYFILE . '.pub', BULK_FTP_KEYFILE)) {
          ...
          if (ssh2_scp_send($conn, $localfile, $urlBulkService . $nombreArchivo)) {
            $code = 0;
            $error_no = 0;
            $logLevel = 'DEBUG';
            $msg = 'Process Ok';
