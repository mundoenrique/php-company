...
$(function () {
  ...
  $('.input-file').each(function () {
    ...
    $(this).on('change', function (element) {
      ...
      fileName
        ? label.addClass('has-file').find('.js-file-name').html(fileName)
  ...
  pendingBulk = $('#pending-bulk').DataTable({
    ...
    columns: [null, null, null, { width: '160px' }, { width: '130px' }],
    columnDefs: [
      {
        targets: 1,
  ...
  $('#system-info').on('click', '.delete-bulk-btn', function (e) {
    ...
    if (formDeleteBulk.valid()) {
      $(this).off('click').html(loader).prop('disabled', true).removeClass('delete-bulk-btn');
      data = {
