...
$(function () {
	...
	if (
		...
		$('#idEnterpriseList>option:selected').attr('countEnterpriseList') == 1
	) {
		enablePhone();
	}
...
function modalDeleteContact(response) {
	...
	inputModal += '<span>';
	inputModal +=
		'¿Esta seguro que quieres eliminar el contacto:<b> ' +
		response.contactNames +
		' ' +
		response.contactLastNames +
		'</b>?';
	inputModal += '<br><br>';
	...
	$('.delete-contact-button').on('click', function (e) {
		...
		if (form.valid()) {
			...
			$(this).prop('disabled', true).removeClass('delete-contact-button');
			insertFormInput(true);
			getCallNovoCoreContact(data, btn);
		}
	});
...
function contactsTable(dataResponse) {
	...
	var tableCont = '';
	tableCont +=
		'<table id="tableContactsCompany" name="tableContactsCompany" class="mt-4 cell-border h6 display w-100 center">';
	...
	$.each(dataResponse, function (index, value) {
		if (value) {
			...
			tableCont +=
				'<button value="' +
				value.id +
				'" class="edit btn mx-1 px-0" title="' +
				lang.GEN_EDIT +
				'" data-action="update" data-toggle="tooltip">';
			tableCont += '<i class="icon icon-edit"></i>';
			tableCont += '</button>';
			tableCont +=
				'<button value="' +
				value.id +
				'" class="delete btn mx-1 px-0" title="' +
				lang.GEN_BTN_DELETE +
