    ...
    onSelect: function (selectedDate) {
      ...
      if (inputDate == 'initialDate') {
        ...
        var maxTime = new Date(
          ...
          dateSelected.getMonth() + lang.SETT_DATEPICKER_MONTHRANGE,
...
function dialog(e) {
  ...
  switch (action) {
    case 'Exportar a EXCEL consolidado':
      ...
      inputModal +=
        '<select name="anio-consolid"class="date-picker-year select-box custom-select ml-1 h6" id="anio-consolid"><option selected value="0">Seleccione año</option></select>';
      inputModal += '</div>';
...
function downloadConsolid(type, oldID) {
  ...
  button.on('click', function () {
    ...
    if (form.valid()) {
      ...
      if (type == 'excel') {
        exportToExcelConsolid(masterAccountdata);
      } else {
        exportToPDFConsolid(masterAccountdata);
      }
    }
  });
}
...
function exportFile(e) {
  ...
  masterAccountdata = {
    ...
    dateStart: $('#initialDate').val(),
    dateEnd: $('#finalDate').val(),
    typeNote: $('input[name=procedure]:checked').val(),
    dateFilter: $("input[name='results']:checked").val(),
    nameEnterprise: $('#enterprise-report').find('option:selected').attr('nomOf'),
    actualPage: '1',
    pageSize: $('#tamP').val(),
  ...
  switch (action) {
    case 'Exportar a EXCEL':
      validateForms(form);
      if (form.valid()) {
        exportToExcel(masterAccountdata);
      }
      break;
    case 'Exportar a PDF':
      validateForms(form);
      if (form.valid()) {
        exportToPDF(masterAccountdata);
      }
      break;
  }
}
...
function exportToExcelConsolid(data, textBtn) {
  who = 'Reports';
...
function exportToPDFConsolid(data) {
  who = 'Reports';
