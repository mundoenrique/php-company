...
$(function () {
	...
	$('#resultado-tarjetas').on('keyup', '.monto', function () {
		...
		if ($(this).val() != '') {
			...
			if ($(this).val() < serv_var.maestroParam.montoMinTransDia) { // se elimina la conversion de tipo a
				$(this).showBalloon({
					position: 'right',
	...
	function calcularTrans(operacion) {
		...
		if (sum = sumaMontosTransferencia(inputsConMontoValidos)) {
			...
			} else if (sum > serv_var.maestroParam.montoMaxTransDia) { // validar montos (diario y semanal)
			...
			} else if ((sum + acumSem) > serv_var.maestroParam.montoMaxTransSemanal) {
			...
			} else if (sum < serv_var.maestroParam.montoMinTransDia) {
	...
	toFormatShow = function (valor) {
		...
		if (isoPais == 'Ve' || isoPais == 'Co') {
			...
			return (isoPais == 'Co' ? '$ ' : 'Bs. ') + formatoNumero(valor, 2, ",", "."); //Comparacion lineal
