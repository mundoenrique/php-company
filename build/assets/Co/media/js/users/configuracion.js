$(function () {
	...
	$('#btn-cambioC').on('click', function () {
		...
		canvas += "<input type=password id='confNew' placeholder='Confirma la nueva contraseña ' maxlength=" + max + " size=26/><h5 id='vacio'></h5></form>";
			...
			buttons: {
				OK: function () {
					...
					if (old == "" || newC == "" || cNewC == "") {
						...
						$(this).find($('#vacio')).text('La nueva contraseña no coincide con la confirmación.');
	...
	function CargarPerfilUser() {
		...
		var dataRequest = JSON.stringify({})
		dataRequest = CryptoJS.AES.encrypt(dataRequest, ceo_cook, { format: CryptoJSAesJson }).toString();
		$.post(baseURL + api + isoPais + '/usuario/config/perfilUsuario', { request: dataRequest, ceo_name: ceo_cook, plot: btoa(ceo_cook) })
			.done(function (response) {
				data = JSON.parse(CryptoJS.AES.decrypt(response.code, response.plot, { format: CryptoJSAesJson }).toString(CryptoJS.enc.Utf8))
				if (!data.ERROR) {
	...
	$('#ui-id-1').on('change', '#listaEmpresas', function () { //seleccionar una empresa
		...
		$("#tlf1").attr('maxlength', '10');
		$("#tlf2").attr('maxlength', '10');
		$("#tlf3").attr('maxlength', '10');
			...
			$.post(baseURL + api + isoPais + '/usuario/config/eliminarContacto', { request: dataRequest, ceo_name: ceo_cook, plot: btoa(ceo_cook) })
				.done(function (response) {
					...
					if (data.rc == "0") {
						...
						$('#contenedor_contacts').empty();
						listarContactos(0);
						$("<div>Proceso exitoso.<h5>Listando contactos...</h5></div>").dialog({ title: "Eliminar contacto", modal: true, close: function () { $(this).dialog('destroy') } });
					} else {
						if (data.rc == '-61' || data.rc == '-29') {
							alert('Usuario actualmente desconectado'); location.reload();
						} else {
				...
				$.post(baseURL + api + isoPais + '/usuario/config/ActualizarContacto', { request: dataRequest, ceo_name: ceo_cook, plot: btoa(ceo_cook) })
					.done(function (response) {
						...
						if (data.rc == "0") {
							...
							$('#contenedor_contacts').empty();
							listarContactos(0);
							$("<div>Proceso exitoso.<h5>Listando contactos...</h5></div>").dialog({ title: "Actualizar contacto", modal: true, close: function () { $(this).dialog('destroy') } });
						} else {
							if (data.rc == '-61' || data.rc == '-29') {
								alert('Usuario actualmente desconectado'); location.reload();
							} else {
	...
	var config_mostrar_form = function (boton, content) {  // BOTON MOSTRAR FORMULARIOS
		$("#lotes-general").on("click", boton, function () {
			...
			$("#lotes-general #agregarContacto input").val('');
			$("#agregarContacto #contact-id").attr('maxlength', '10');
			$(this).hide();
		});
	}
