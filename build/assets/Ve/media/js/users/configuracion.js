        ...
        if(!$("#pass").val()==""){
            ...
            $.post(baseURL+api+isoPais+'/usuario/config/eliminarContacto', json).done(function( data ) {
                ...
                if(data.rc=="0"){
                    ...
                    $('#contenedor_contacts').empty();
                    listarContactos(0);
                    $("<div>Proceso exitoso.<h5>Listando contactos...</h5></div>").dialog({title:"Eliminar contacto",modal:true, close:function(){$(this).dialog('destroy')}});//notificacion("Eliminar contacto","Proceso exitoso.");
                }else{
                    if(data.rc=='-61'||data.rc=='-29'){
                        alert('Usuario actualmente desconectado'); location.reload();
                    }else{
            ...
            if(!$("#pass").val()==""){
                ...
                $.post(baseURL+api+isoPais+'/usuario/config/ActualizarContacto', json).done(function( data ) {
                    ...
                    if(data.rc=="0"){
                        ...
                        $('#contenedor_contacts').empty();
                        listarContactos(0);
                        $("<div>Proceso exitoso.<h5>Listando contactos...</h5></div>").dialog({title:"Actualizar contacto",modal:true, close:function(){$(this).dialog('destroy')}});//notificacion("Actualizar contacto","Proceso exitoso.");
                    }else{
                        if(data.rc=='-61'||data.rc=='-29'){
                            alert('Usuario actualmente desconectado'); location.reload();
                        }else{
...
//--Fin POUP Notificacion
    ...
    function listarContactos(paginaActual){
        ...
        if(!rif ==""){
            ...
            $.post(baseURL+api+isoPais+'/usuario/config/InfoContactoEmpresa', {'data-rif':rif, "paginaActual":paginaActual, 'data-paginar':pagina, 'data-cantItems':cantItems })
                .done(function( data ) {
