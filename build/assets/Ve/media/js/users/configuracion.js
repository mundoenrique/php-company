$(function () {
	...
	$('#btn-cambioC').on('click', function () {
		...
		canvas += "<input type=password id='confNew' placeholder='Confirma la nueva contraseña ' maxlength=" + max + " size=26 class='required'/><h5 id='vacio'></h5></form>";
	...
	function CargarPerfilUser() {
		...
		$.post(baseURL + api + isoPais + '/usuario/config/perfilUsuario', {
			request: dataRequest,
			ceo_name: ceo_cook,
			plot: btoa(ceo_cook)
	...
	$('#ui-id-1').on('change', '#listaEmpresas', function () { //seleccionar una empresa
		...
		$("#tlf1").attr('maxlength', '11');
		$("#tlf2").attr('maxlength', '11');
		$("#tlf3").attr('maxlength', '11');
		...
		$.post(baseURL + api + isoPais + '/usuario/config/infoEmpresa', {
			request: dataRequest,
			ceo_name: ceo_cook,
			plot: btoa(ceo_cook)
		}).done(function (response) {
	...
	$("#lotes-general").on("click", "#agregar", function () { // boton agregar el contecto
		...
		if (validar(json, $("#agregarContacto"), "agregar") && $("#passAgregar").val() !== "") {
			...
			if (!rif == "") {
				...
				$.post(baseURL + api + isoPais + '/usuario/config/agregarContacto', {
					request: dataRequest,
					ceo_name: ceo_cook,
					plot: btoa(ceo_cook)
				}).done(function (response) {
	...
	$("#lotes-general").on("click", "#modif", function () {
		...
		if (tlfRegex.test($("#tlf1").val()) && (tlfRegex.test($("#tlf2").val()) || $("#tlf2").val() == "") && (tlfRegex.test($("#tlf3").val()) || $("#tlf3").val() == "")) {
			...
			$.post(baseURL + api + isoPais + '/usuario/config/ActualizarTlfEmpresa', {
				request: dataRequest,
				ceo_name: ceo_cook,
				plot: btoa(ceo_cook)
			}).done(function (response) {
		...
		if (!$("#pass").val() == "") {
			...
			$.post(baseURL + api + isoPais + '/usuario/config/eliminarContacto', { request: dataRequest, ceo_name: ceo_cook, plot: btoa(ceo_cook) }).done(function (response) {
				...
				if (data.rc == "0") {
					...
					}); //notificacion("Eliminar contacto","Proceso exitoso.");
				} else {
					if (data.rc == '-61' || data.rc == '-29') {
						alert('Usuario actualmente desconectado');
						location.reload();
			...
			if (!$("#pass").val() == "") {
				...
				$.post(baseURL + api + isoPais + '/usuario/config/ActualizarContacto', { request: dataRequest, ceo_name: ceo_cook, plot: btoa(ceo_cook) }).done(function (response) {
					...
					if (data.rc == "0") {
						...
						}); //notificacion("Actualizar contacto","Proceso exitoso.");
					} else {
						if (data.rc == '-61' || data.rc == '-29') {
							alert('Usuario actualmente desconectado');
							location.reload();
	...
	$("#lotes-general").on("click", ".OS-icon", function () {
		...
		$.each(datos.lista, function (pos, item) {
			if (obj == item.cod) {
				...
				ciudades = datos.paisTo.listaEstados.filter(function (dat) {
					return dat.codEstado == item.estado
				});
				$('#suc_ciudad').empty();
	...
	$('#ui-id-2').on('change', '#suc_estado', function () {
		...
		ciudades = datos.paisTo.listaEstados.filter(function (dat) {
			return dat.codEstado == $("option:selected", '#suc_estado').val()
		});
		$('#suc_ciudad').empty();
		$.each(ciudades[0].listaCiudad, function (pos, val) {
			$('#suc_ciudad').append('<option value="' + val.codCiudad + '">' + val.ciudad + '</option>');
	...
	$("#lotes-general").on("click", "#agregarSuc", function () {
		...
		if (validez && $("#pass_suc").val() != "") {
			...
			$.post(baseURL + api + isoPais + '/usuario/config/agregarSucursales', {
				request: dataRequest,
				ceo_name: ceo_cook,
				plot: btoa(ceo_cook)
			}).done(function (response) {
				...
				if (data.rc == "0") {
					...
					notificacion("Agregar sucursal", "Proceso exitoso.");
				} else {
					if (data.rc == '-61' || data.rc == '-29') {
						alert('Usuario actualmente desconectado');
						location.reload();
	...
	$("#lotes-general").on("click", "#btn-modif-suc", function () {
		...
		if (validez && $("#pass_suc").val() != "") {
			...
			$.post(baseURL + api + isoPais + '/usuario/config/actualizarSucursales', {
				request: dataRequest,
				ceo_name: ceo_cook,
				plot: btoa(ceo_cook)
			}).done(function (response) {
	...
	function consultarSucursales(paginaActual) {
		...
		$.post(baseURL + api + isoPais + '/usuario/config/consultarSucursales', {
			request: dataRequest,
			ceo_name: ceo_cook,
			plot: btoa(ceo_cook)
		}).done(function (response) {
	...
	function cargarPais(data) {
		...
		ciudades = data.paisTo.listaEstados.filter(function (dat) {
			return dat.codEstado == $("option:selected", "#suc_estado").val()
		});
		$('#suc_ciudad').empty();
		$.each(ciudades[0].listaCiudad, function (pos, val) {
	...
	$("#lotes-general").on("click", "#mostrarContact", function () {
		// boton agregar contacto a empresa
				...
				.done(function (response) {
					...
					if (data.rc == '0') {
						...
						$.each(data.lista, function (k, v) {
							...
							$canvas += '<a title="' + $('#title_modificar').val() + '"><span id="contact-idInput" class=""></span>';
						...
						notificacion('Contactos', 'No se encontraron datos de contactos para esta Empresa');
					} else {
						if (data.rc == '-61' || data.rc == '-29') {
							alert('Usuario actualmente desconectado');
							location.reload();
