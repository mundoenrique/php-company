...
$(document).ready(function () {
	...
	$.getJSON(baseURL + api + isoPais + '/empresas/consulta-empresa-usuario').always(function (response) {
		...
		if (!(data.ERROR)) {
			...
			$.each(data.lista, function (k, v) {
				...
				$("#Reporte-tarjeta-hambiente").append('<option value="' + v.accodcia + '" acnomcia="' + v.acnomcia + '" acrazonsocial="' + v.acrazonsocial + '" acdesc="' + v.acdesc + '" acrif="' + v.acrif + '">' + v.acnomcia + '</option>');
			});
		} else {
	...
	$("#Reporte-tarjeta-hambiente").on("change", function () {
		...
		if (acrif) {
			...
			$.post(baseURL + api + isoPais + "/reportes/consulta-producto-empresa", {
				request: dataRequest,
				ceo_name: ceo_cook,
				plot: btoa(ceo_cook)
			}, function (response) {
