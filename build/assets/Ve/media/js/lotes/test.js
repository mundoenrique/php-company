...
$(function () { // Document ready
	...
	$("#buscar").on("click", function () {
		...
		$.post(baseURL + api + isoPais + '/lotes/reproceso/buscar', {
				'data-tipo': tipoBuskr,
				ceo_name: ceo_cook
			})
			.done(function (data) {
	...
	$("#reprocesar").on('click', function () {
		...
		$aux = $('#loading').dialog({
			title: 'Generando Orden de Servicio',
			close: function () {
				$(this).dialog('close');
			},
			modal: true,
		...
		$.post(baseURL + api + isoPais + "/lotes/reproceso/reprocesar", {})
			.done(function (data) {
				...
				if (data.indexOf("ERROR") == -1) {
					$("<div><h3>Proceso exitoso</h3><h5>Redireccionando...</h5></div>").dialog({
						title: "Reprocesar lotes",
						modal: true,
						resizable: false,
						close: function () {
							$(this).dialog('destroy');
