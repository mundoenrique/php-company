$(function () {
	...
	function buscar(pgSgt) {
		...
		var dataRequest = JSON.stringify({ 'data_dni': $('#dni').val(), 'data_tjta': $('#nroTjta').val(), 'data_pg': pgSgt, 'data_paginas': serv_var.paginas, 'data_paginar': serv_var.paginar });
	...
	function cargarResultado(data) {
		...
		if (data.result.listadoTarjetas.lista.length > 0) {
			...
			$.each(data.result.listadoTarjetas.lista, function (k, v) {
				...
				tr += '<td id="td-nombre-2">' + v.NombreCliente.toLowerCase().replace(/(^| )(\w)/g, function (x) { return x.toUpperCase(); }) + '</td>';
	...
	function calcularTrans(operacion) {
		...
		if (sum = sumaMontosTransferencia(inputsConMontoValidos)) {
			...
			if ((cantxdia + tjtas) > maxTransDia) { // validaciones de cantidad trans por dia
				...
				notificacion(trans + ' a tarjeta', '<h2 style="line-height:20px">Has excedido el monto máximo de transacción</h2> <h6>Monto ' + trans + ': ' + toFormatShow(sum) + '</h6><h6>Monto permitido: ' + toFormatShow(serv_var.maestroParam.montoMaxTransaccion) + '</h6>')
				return false;
