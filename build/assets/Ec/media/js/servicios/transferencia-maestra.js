...
function llamarWS(pass, url, operacion, mensaje) {
	...
	.done(function(data) {
		...
		if (!data.ERROR) {
			...
			notificacion(
				mensaje,
				'<h4>Proceso exitoso</h4><h5>' + serv_var.fallidas + ' tarjetas fallidas</h5><h5>Verifique estatus y/o saldo de sus tarjetas</h5>'
			);
		} else {
			if (data.ERROR == '-29') {
				alert('Usuario actualmente desconectado');
...
function toFormat(valor) {
	...
	if (valor.length > 0) {
		...
		} else if (isoPais == 'Ve' || isoPais == 'Co' || isoPais == 'Ec') {
