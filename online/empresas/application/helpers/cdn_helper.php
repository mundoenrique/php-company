...
if ( ! function_exists('insert_js_cdn'))
{
	...
	function insert_js_cdn($filename = '')
	{
		...
		$js='<script src="'.$url_cdn.'media/js/'.$filename.'?20171206" type="text/javascript"></script>';
		return $js;
	}
}
...
if ( ! function_exists('insert_css_cdn'))
{
	...
	function insert_css_cdn($filename = '')
	{
		$CI =& get_instance();
		$url_cdn = $CI->config->item('base_url_cdn');
		$css='<link rel="stylesheet" type="text/css" href="'.$url_cdn.'media/css/'.$filename.'?20171206"/>';
		return $css;
	}
}
...
if ( ! function_exists('insert_image_cdn'))
{
	...
	function insert_image_cdn($filename = '')
	{
		$CI =& get_instance();
		$url_cdn = $CI->config->item('base_url_cdn');
		$css='<img src="'.$url_cdn.'media/img/'.$filename.'?20171206">';
		return $css;
	}
}
